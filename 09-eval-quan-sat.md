...
2. **Hệ thống có khỏe không?** (quan sát: khi đang chạy)

Không có đường cơ sở thì mọi thay đổi sau này đều không chứng minh được là cải thiện hay làm tệ đi. Vì vậy **bộ câu hỏi đánh giá được làm trước code RAG** (M1).
...
| `question`, `locale` | Câu hỏi thật. **Hai ngôn ngữ:** tiếng Pháp là chính, tiếng Anh là thứ hai (GĐ-1). `locale` là trường bắt buộc để **tách số recall theo ngôn ngữ**; một số gộp sẽ che việc nhánh từ khóa yếu đi với câu hỏi tiếng Anh (R42) |
...
**Kế hoạch tăng trưởng:** 50 câu ở M1 (đủ để đo đường cơ sở), 100+ ở tuần 6; mọi câu trả lời "xấu" từ beta được kỹ sư phân loại rồi bổ sung vào bộ (luồng F5 bên dưới).
...
| --- | --- | --- | --- |
| **Tầng 1: tất định** | Recall@k và MRR của truy xuất so với `expected_chunks`; trích dẫn trong câu trả lời phải ứng với chunk đã truy xuất; số trong câu trả lời tool phải khớp **engine chạy độc lập**; từ chối đúng nhóm B; cách ly tenant | **Không** | **Có** |
...
sequenceDiagram
  ...
  participant DB as PostgreSQL (kho eval)
  ...
  EV->>DB: Nạp kho eval cố định (chunk mẫu đã embedding)
...
**Đánh giá "kho mới" trước khi phát hành:** mỗi lần nạp tài liệu (F4) chạy eval nhanh (10 câu neo) trước khi chuyển `active` ([03](03-rag.md) §1). Đổi mô hình embedding chạy eval đầy đủ ở chế độ bóng ([03](03-rag.md) §4).
...
Nếu nền tảng chạy (ECS/EKS/EC2) được chốt là hỗ trợ, có thể bật **Application Signals** cho .NET để có service map và SLO; skill AWS Observability có hướng dẫn cho .NET trên EC2/ECS/EKS/Lambda. Chọn sau khi chốt topology ([10](10-trien-khai.md), GĐ-7).
...
| Lỗi ingestion | Bất kỳ job `Failed` | Kho không được cập nhật |
...
flowchart TD
  ...
  K -->|"Tìm kiếm sót đoạn đúng"| RET["Lỗi truy xuất: sửa cắt đoạn / chỉ mục / hybrid"]
...
Sách Building Gen AI Applications with Amazon Bedrock đề xuất lược đồ đủ để dựng lại mỗi lần gọi. Bản ở đây giữ tinh thần đó nhưng **không lưu nội dung**, để khớp retention ([07](07-auth-bao-mat.md) §6):
...
| `prompt_template_id` + version | Prompt nằm trong repo có version ([10](10-trien-khai.md) §7) |
...
| --- | --- | --- |
| **Đánh giá trực tuyến** | Không chặn người dùng; chạy nền, lấy mẫu 5–10% lượt hỏi thật (đủ ý nghĩa thống kê); ngoại tuyến để tinh chỉnh, trực tuyến để cảnh báo trôi dạt | Job nền của `Assistant.Worker` chấm mẫu ngẫu nhiên các `message` đã hoàn tất, chỉ trên dữ liệu người dùng đã được thông báo ([07](07-auth-bao-mat.md) §6) |
...
| **Kiểm tra tài liệu hết hiệu lực** | Tài liệu bị thay thế mà chưa gỡ vẫn được truy xuất như bản hiện hành | Assertion Tầng 1: sau khi chuyển một tài liệu sang `superseded`, không chunk nào của nó xuất hiện trong kết quả truy xuất; kiểm tra xung đột định danh tiêu chuẩn trùng nhau |
...
| **Chẩn đoán theo thứ tự** | Truy xuất → sinh → nạp; recall 40% thì không tinh chỉnh prompt | Đã là quy tắc của cổng G1 |
...
**Ngưỡng chất lượng production tham chiếu (ví dụ của sách, không phải cam kết):** context precision ≥ 0,9; context recall ≥ 0,8; tỉ lệ ảo giác ≤ 0,05; answer relevance ≥ 0,9. Đây là điểm để đặt mục tiêu **sau beta** khi đã có số đo thật ([11](11-lo-trinh.md) §7); ngưỡng nghiệm thu 8 tuần vẫn là bảng ở [11](11-lo-trinh.md) §3.
...
| **Nhịp cải tiến** | Enterprise GenAI | Hằng đêm: eval tự động; hằng tuần: phân loại phản hồi ([09](09-eval-quan-sat.md) §5); hai tuần một lần: cập nhật truy xuất/prompt theo sprint |
...
| **Nhất quán không đồng nghĩa với đúng** | Agents in Action | Chỉ số nhất quán ([09](09-eval-quan-sat.md) §8) là điều kiện cần, không thay cho đối chiếu golden |
...
| **Ma trận truy vết yêu cầu → kiểm thử (RTM)** | Enterprise GenAI | [11](11-lo-trinh.md) §9 |
...
| **Ghi đè của người dùng trong audit** | Interpretable AI | Ghi cả "Bỏ qua" và chỉnh tham số ([07](07-auth-bao-mat.md) §9) |

### Bổ sung sau khi đọc phần khung của 9 cuốn

| Chủ đề | Nguồn | Áp dụng |
| --- | --- | --- |
| **Mỗi thành phần phức tạp phải thắng phương án đơn giản hơn** | Interpretable AI (Rashomon), Enterprise GenAI (LLM fit) | Chạy loạt so sánh truy xuất trên golden set: chỉ BM25, cộng vector, cộng RRF, cộng rerank, cộng viết lại câu hỏi. Thành phần nào không tăng recall@k hoặc nDCG quá ε thì bỏ (rerank còn phải tính thêm độ trễ). Loạt này không gọi LLM nên rẻ; chạy ở tuần 3, **trước G1**, làm bằng chứng cho cổng này |
| **Nhóm câu hỏi về kết quả bất ngờ** | Interpretable AI | Vài trường hợp engine cho số ngoài dự kiến; giám khảo kiểm lời diễn giải không bịa nguyên nhân ngoài output tool ([04](04-tool-calling.md) §7) |
| **`tool_repeat_rate`** | AI Agents in Action | Tỉ lệ lượt có lời gọi lặp cùng tool cùng tham số ([04](04-tool-calling.md) §6); tăng bất thường sau đổi prompt hoặc mô tả tool là dấu hiệu hồi quy |
| **Định vị lỗi theo Plan / Action / Observation** | Building Gen AI Applications with Amazon Bedrock | Khi một lượt sai, gắn nhãn khâu hỏng: chọn/lập kế hoạch sai, gọi tool sai tham số, hay hiểu sai kết quả tool. Ba nhãn này khớp bảng phân loại thất bại ở [04](04-tool-calling.md) §7 và giúp biết sửa mô tả tool, ToolGate hay prompt diễn giải |

### Quan sát trên AgentCore Harness (AD-13)

| Chủ đề | Áp dụng |
| --- | --- |
| Trace | AgentCore Observability tự phát trace cho model, tool và bước nhớ; cần bật Transaction Search của CloudWatch một lần cho tài khoản — **việc của tuần 1**, vì không có nó thì V-A3 và V-A4 không đọc được số. `Assistant.Api` truyền `traceparent` vào `InvokeHarness` để nối span của lượt hỏi ([09](09-eval-quan-sat.md) §4) với trace của Harness |
...
| Đánh giá | Dịch vụ AgentCore Evaluations có ở Frankfurt nhưng tài liệu chỉ nêu hỗ trợ trace của Strands và LangGraph. Chưa dùng; giữ bộ eval riêng (giám khảo Opus 5, golden set, cổng CI) vì cần cổng chặn merge và bộ câu hỏi kỹ thuật tiếng Pháp |
...
| Chủ đề | Nguồn | Áp dụng |
| --- | --- | --- |
| **Truy vấn dựng bằng mã và truy vấn do model viết** | Câu hỏi về kết hợp tính toán với tài liệu | Nhóm J có câu hỏi mà điều khoản chi phối đã biết; so recall của điều khoản đó giữa hai cách. Nếu truy vấn dựng bằng mã không thắng thì bỏ tool `standards.find_basis` ([04](04-tool-calling.md) §11) |
