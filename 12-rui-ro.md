...
| Q4 | **Topology production:** nền tảng chạy container là gì? Có PostgreSQL production hỗ trợ `pgvector` không, hay cần instance mới? | Cập nhật [10](10-trien-khai.md); chọn phương án quan sát; chi phí | Cuối tuần 1 |
...
| R14 | Build HNSW/tải vector làm chậm PostgreSQL dùng chung | V | C | Instance riêng cho assistant (AD-12) | L |
...
| R22 | HNSW kết hợp bộ lọc `scope_key` bỏ sót kết quả của tài liệu thuộc một organization, không báo lỗi | V | C | `hnsw.iterative_scan` hoặc overfetch; đo recall theo từng scope; nhóm eval G ([03](03-rag.md) §2, [09](09-eval-quan-sat.md) §1) | B1 |
...
| Hạng mục | Trạng thái |
| --- | --- |
| Managed Knowledge Base ở `eu-central-1` (khả dụng, hành vi lọc quyền, chunking tùy biến) | Chưa xác minh. AD-04 chọn tự xây vì các lý do độc lập với việc này; ghi nhận ở M0 |
