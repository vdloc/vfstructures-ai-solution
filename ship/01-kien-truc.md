...
| AD-31 | **Cổng MCP cho ứng dụng ngoài đặt trên AgentCore Gateway.** Cùng tool facade, cùng `scope` và RLS theo người dùng trong token, cùng quota và `audit_event`. Token phải có `aud` đúng endpoint MCP và **không** được chuyển tiếp xuống dưới: facade nhận token hẹp quyền qua RFC 8693 như đường trong app. Cổng chỉ trả dữ liệu có nguồn — `tool_result` kèm tham số, chunk kèm citation — **không** trả văn bản do mô hình sinh | MCP server tự viết (trái AD-08); mở thẳng Main API cho client ngoài; dùng lại token của client cho lời gọi phía sau | Thêm một đường vào phải giám sát; mỗi tool lộ ra ngoài phải duyệt riêng; mô hình phía client nằm ngoài guardrail và validator của hệ thống |
...
**Ràng buộc kèm theo AD-31:** danh sách tool lộ ra cổng MCP khai riêng trong `tools.manifest.yaml`; mặc định là **không lộ**.
