...
| AgentCore Gateway | Gateway biến API, Lambda và dịch vụ sẵn có thành *"Model Context Protocol (MCP)-compatible tools"*; quản lý cả xác thực vào lẫn xác thực ra trong một dịch vụ | https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/gateway.html | 23/09/2026 |
| AgentCore Gateway · giao thức MCP | Gateway phục vụ MCP bản 2026-07-28, 2025-11-25, 2025-06-18, 2025-03-26; có `tools/list`, `tools/call`, prompts, resources, elicitation, sampling, và tìm tool bằng câu hỏi tự nhiên | https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/gateway-using.html | 23/09/2026 |
| AgentCore Gateway · xác thực vào | *"Amazon Bedrock AgentCore supports JWTs from all identity providers"*; `customJWTAuthorizer` nhận `discoveryUrl`, `allowedClients`, `allowedAudience`, `allowedScopes`, và `privateEndpoint` cho IdP trong VPC; thiếu token thì trả **401** kèm `WWW-Authenticate` có `resource_metadata` trỏ `/.well-known/oauth-protected-resource` (RFC 9728); `NONE` và `AUTHENTICATE_ONLY` để cổng **không** tự phân quyền; CloudTrail lưu claim `sub` | https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/gateway-inbound-auth.html | 23/09/2026 |
| OpenSearch Serverless · capacity | Mỗi OCU = 6 GiB bộ nhớ kèm vCPU; đặt tối thiểu khác 0 để tránh cold start; bậc OCU đặt được là 2, 4, 8, 16… | https://docs.aws.amazon.com/opensearch-service/latest/developerguide/serverless-scaling.html | 23/09/2026 |
